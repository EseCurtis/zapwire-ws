...
const handlerouters=()=>{
    ...
    for(i=0;i<links.length;i++){
        let activeClass = links[i].getAttribute('is-active') || 'active';
    ...
    for(i=0;i<vForms.length;i++){
        if(vForms[i].getAttribute('async') == 'true'){
            vForms[i].onsubmit = (event)=>{
                ...
                let vForm       = new FormData(event.target);
                ...
                if(event.currentTarget.getAttribute("waitload")) eval(event.currentTarget.getAttribute("waitload"))
                loader.start()

                ajax(vForm, submitUrl, eval("loader.stop(),"+event.currentTarget.getAttribute("callback")));
            };
        }
    }
    return 1;
