 ...
 */
    ...
    $visible_project_info  = $amvc->project_info['visible'];
    $new_project_info = array();

    foreach ($visible_project_info as $key => $value) {
        $new_project_info[$value] = $amvc->project_info[$value];
    }
        
    $project_info = json_encode($new_project_info, TRUE); 

    //define the scripts an their paths
    $amvc_scripts = [
        "page_data" => [
            "title" =>"Page data",
            "code"  =>"let amvcPageData = {
                view:'".end($amvc->values)."', 
                siteUrl :'"._URL_."', 
        ...
        "amvc.icons.js"  => [
            "title" => "AMVC SVG icons [amvc.icons.js]",
            "code"  => file_get_contents("$r_path/bin/js/amvc.icons.js")
        ],
        "amvc.functions.js"  => [
